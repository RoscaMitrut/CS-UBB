...
%token PROG LPAREN RPAREN BEGINNING END VAR INT COLON
%token SEMICOLON PLUS MINUS MUL DIV
...
main_function: PROG IDENTIFIER SEMICOLON declaration BEGINNING instructions ENDPROG {
        defineDefaults();
        sprintf(
            $$,
            "\tstart:\n%s\n"
...
declaration: VAR declaration_sequence COLON declaration_type SEMICOLON {    
...
expression: expression_factor {
    ...
    | expression PLUS expression_factor {
    ...
    | expression MINUS expression_factor {
        int tempVarIdx = newTempVar();
        sprintf($$.varname, "[%s + %d]", tempArray, 4 * tempVarIdx);
        sprintf(
            $$.code,
...
input_statement:  READLN LPAREN input_sequence RPAREN {
...
input_sequence: IDENTIFIER COMMA input_sequence {
...
output_statement: WRITELN LPAREN output_sequence RPAREN{
...
output_sequence: output_term COMMA output_sequence {
