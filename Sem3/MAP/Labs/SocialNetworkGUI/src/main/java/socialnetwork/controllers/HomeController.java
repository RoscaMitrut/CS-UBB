...
public class HomeController extends AbstractController{
    ...
    @FXML
    Button removeButton, addButton, accountSettingsButton, logOutButton, chatButton, butonInainte, butonInapoi;

    @FXML
    TextField searchUserTextField,textFieldPaginare;
    ...
    public void setNrPePagina(KeyEvent keyEvent){
        try{
            nrPePagina = Integer.parseInt(textFieldPaginare.getText());
    ...
    private void handleFilter() {
        ...
        if (requestsCheckBox.isSelected()) {
            ...
            usersModel.setAll(getUsersPaginat()
                    .stream()
                    .filter(predicate)
                    .collect(Collectors.toList()));
        } else {
    ...
    @FXML
    protected void inainteButonClicked(){
        pagina+=1;
        butonInapoi.setDisable(false);
        update();
    }

    @FXML
    protected void inapoiButonClicked(){
        if(pagina>1) {
            pagina -= 1;
        }
        if(pagina==1){
            butonInapoi.setDisable(true);
        }
        update();
    }
    ...
    private List<UserDTO> getUsersPaginat(){
        Integer offset = (pagina-1)*nrPePagina;
        Iterable<User> usersIterable = this.networkService.getAllUsersPaginat(nrPePagina,offset);
        List<User> usersList =
                StreamSupport.stream(usersIterable.spliterator(), false).toList();
